...
jQuery(document).ready(function() {
	var $ = jQuery;
    var $svg = $('.sidebar'),
        $demo = $('.elastic-sidebar'),
        $path = $('.s-path'),
        $sCont = $('.elastic-sidebar-box'),
        ...
        finalX = 0,
        frame = 1000 / 60,
        animTime = 600,    // скорость анимации
        sContTrans = 200,  // пауза перед схлапыванием сайдбара
        animating = false;
    var easings = {
    ...
    function handlers1() {
        $(document).on('mousedown touchstart', '.s-path', function(e) {
            ...
            $(document).on('mousemove touchmove', function(e) {
                ...
                $('body').css('left', diffX + 'px');
            });
			$('.elastic-sidebar').css({'width': '30em'});
        });
    ...
    handlers1();
	function explode(){
		$('.elastic-sidebar').css({'width': '15px'});
        $('body').css('left', '0px');
	}
    function closeSidebar(e) {
        if ($(e.target).closest('.elastic-sidebar-box').length) return;
        if (animating) return;
        animating = true;
